...
module.exports = function(serverStatus, message, config){
  ...
  return {
    connect: function(){
      ...
      // check for csrf-token and include in query params
      var pageCsrfToken = document.querySelector(config.csrfTokenSelector) ? document.querySelector(config.csrfTokenSelector).getAttribute(config.csrfTokenAttribute) : null;
      if (pageCsrfToken && config.csrfTokenQueryParam) {
        if (!config.query) config.query = {}
        config.query[config.csrfTokenQueryParam] = pageCsrfToken
      }

      // it is possible to change the engine.io path based on http parameter 'proxy-path' (for proxied connection)
