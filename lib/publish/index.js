    ...
    api: function(transport) {
      ...
      methods = {
        local: function() {
          ...
          transport.send(obj, true); // specify it should go to local 'all' channel
          if (!isInternal(event)) {
            return console.log('⇠'.green + ' event:all'.grey + ' ' + event);
          }
        },
        all: function() {
          var event, obj, params;
        ...
        socketId: function() {
          ...
          transport.send(obj, true); // socketId requests are always sent local and not via rabbitmq
          return console.log('⇠'.green + (" event:socketId:" + socketId).grey + ' ' + event);
        },
        users: function() {
          ...
          transport.send(obj);
          return console.log('⇠'.green + (" event:users:[" + (users.join(',')) + "]").grey + ' ' + event);
        },
        channels: function() {
          ...
          transport.send(obj);
          return console.log('⇠'.green + (" event:channels:[" + (channels.join(',')) + "]").grey + ' ' + event);
        }
      };
