...
var app, connect, connectSession, serveStatic, eventMiddleware, existslib, fileUtils, fs, loadStaticDirs, pathlib, router, settings, staticDirs, staticFiles, transformURL, useAfterStack;
...
module.exports = function(root) {
  return {
    ...
    load: function(staticPath, sessionStore, sessionOptions) {
      ...
      // Initialize the official express-session middleware
      var sessionMiddleware = connectSession({
        name: sessionOptions.httpCookie,
        cookie: {
          path: '/',
        ...
        secret: sessionOptions.cookieSecret,
        store: sessionStore,
        resave: false,
      ...
      app.use(function(req, res, next) {
        sessionMiddleware(req, res, function(err) {
          ...
          // If fallback is enabled, set cookie SameSite=None to Lax for insecure requests (e.g. internal health checks, non-SSL proxy)
          // to prevent browsers from rejecting cookies when 'secure' evaluates to false
          // https://github.com/expressjs/session/issues/725
          if (sessionOptions.sameSiteFallback &&
      ...
      app.use(eventMiddleware).use(serveStatic(staticPath, settings["static"]));
