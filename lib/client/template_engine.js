...
module.exports = function(ss) {
  ...
  return {
    ...
    // added a flag to indicate if template will be included in external js (may need to include/exclude script tags)
    generate: function(dir, files, isExternalJs, cb) {
      ...
      return files.forEach(function(path) {
        ...
        try {
          return formatter.compile(fullPath, {}, function(output) {
            // added externalScript arg.
            templates.push(wrapTemplate(output, path, fullPath, engine, prevEngine, isExternalJs));
            prevEngine = engine;
            ...
            if (templates.length === files.length) {
              ...
              if (engine !== null && engine.suffix) {
                output += engine.suffix(isExternalJs);
              }
              return cb(output);
            }
          });
...
// added 'isExternal' bool to indicate if template will be included in html or externally (ie. do we need to wrap in script tags for inline html use)
wrapTemplate = function(template, path, fullPath, engine, prevEngine, isExternalJs) {
  var output;
  output = [];
  ...
  if (prevEngine && prevEngine !== engine && prevEngine.suffix) {
    output.push(prevEngine.suffix(isExternalJs));
  }
  ...
  if ((prevEngine === null || prevEngine !== engine) && engine.prefix) {
    output.push(engine.prefix(isExternalJs));
  }
