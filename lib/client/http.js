...
module.exports = function(ss, clients, options) {
  ...
  res.serveClient = function(name, locals, statusCode) {
    ...
    try {
      ...
      // cache the data returned from viewJade and on subsequent calls (ie. cached) call the jade parser method with the cached options (+ new locals)
      if (options.packedAssets) {
        if (!cache[name] || (options.locals && options.locals.nonce)) {
        ...
        // Mix-in locals to cached template
        return renderJade({path: cache[name].path, jade: cache[name].jade, options: misc.extend({}, cache[name].options, {locals: options.locals || {}})}, sendHTML);
      }
      else {
        return viewJade(ss, client, options, function(path, jade, opts) {
