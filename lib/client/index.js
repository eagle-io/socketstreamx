...
module.exports = function(ss, router) {
  ...
  return {
    ...
    load: function() {
      var client, id, name, pack, jsIntegrity, integrityPath;
      ...
      if (options.packedAssets) {
        // Force Repack
        if (options.packedAssets.forcePack) {
          packAssets = 1
        }
        ...
        // path to source integrity hash file which gets created during asset packing
        integrityPath = path.join(ss.root, options.dirs.assets) + '/integrity.json'
        ...
        if (packAssets) {
          ...
          for (name in clients) {
            client = clients[name];
            // optionally use fixed asset id (filename)
            if (options.packedAssets.id) {
              client.id = options.packedAssets.id
            }
            pack(ss, client, options, jsIntegrity);
          }
          // write integrity hashes to file
          fs.writeFileSync(integrityPath, JSON.stringify(jsIntegrity, null, 2));
          ss.log('✓'.green +  ' Creating integrity hash file: ' + integrityPath)
        }

        // read integrity hashes from file if it exists
        try {
          options.packedAssets.integrity = JSON.parse(fs.readFileSync(integrityPath).toString());
        } catch (err) {
          ss.log('!'.red + (" Unable to read integrity hashes from file '" + integrityPath + "': ") + err.message.grey);
        }
