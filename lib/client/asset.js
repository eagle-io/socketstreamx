...
module.exports = function(ss, options) {
  ...
  loadFile = function(dir, fileName, type, options, cb) {
    ...
    if (!formatter && ['md', 'map'].indexOf(extension.toLowerCase()) >= 0) {
      return cb("");
    }
    if (!formatter) {
      throw new Error("Unsupported file '" + fileName + "'' with extension '." + extension + "' when we were expecting some type of " + (type.toUpperCase()) + " file. Please provide a formatter for " + (path.substring(root.length)) + " or move it to /client/static");
    }
