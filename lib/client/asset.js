...
var formatKb, formatters, fs, log, minifyJSFile, pathlib, uglifyjs, wrap, wrapCode,
...
module.exports = function(ss, options) {
  ...
  loadFile = function(dir, fileName, type, options, cb) {
    ...
    if (!formatter && ['ts', 'md', 'map'].indexOf(extension.toLowerCase()) >= 0) {
      return cb("");
    }
    if (!formatter) {
      throw new Error("Unsupported file '" + fileName + "'' with extension '." + extension + "' when we were expecting some type of " + (type.toUpperCase()) + " file. Please provide a formatter for " + (path.substring(ss.root.length)) + " or move it to /client/static");
    }
...
minifyJSFile = function(originalCode, fileName) {
  var ast, minified;
  try {
    minified = uglifyjs.minify(originalCode, {fromString: true, output: {max_line_len: false}});
    log(("  Minified " + fileName + " from " + (formatKb(originalCode.length)) + " to " + (formatKb(minified.code.length))).grey);
  }
  catch (e) {
    log(("  Minify FAILED " + fileName).grey);
    throw new Error(e)
  }
